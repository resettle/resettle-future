...
userRouter.post(
  GLOBAL_API_SCHEMAS.user.createUsers.route.path,
  jsonValidator(GLOBAL_API_SCHEMAS.user.createUsers.body),
  async ctx => {
    const db = ctx.get('db')
    ...
    return apiSuccessResponse(
      GLOBAL_API_SCHEMAS.user.createUsers.responseData,
      results.map(r => ({
        id: r.id,
        username: r.username,
        metadata: r.metadata,
...
userRouter.patch(
  GLOBAL_API_SCHEMAS.user.updateUsers.route.path,
  jsonValidator(GLOBAL_API_SCHEMAS.user.updateUsers.body),
  async ctx => {
    const db = ctx.get('db')
    ...
    return apiSuccessResponse(
      GLOBAL_API_SCHEMAS.user.updateUsers.responseData,
      results.map(r => ({
        id: r.id,
        username: r.username,
        metadata: r.metadata,
...
userRouter.post(
  GLOBAL_API_SCHEMAS.user.deleteUsers.route.path,
  jsonValidator(GLOBAL_API_SCHEMAS.user.deleteUsers.body),
  async ctx => {
    const db = ctx.get('db')
    const results = await db.transaction().execute(async tx => {
    ...
    return apiSuccessResponse(
      GLOBAL_API_SCHEMAS.user.deleteUsers.responseData,
      results,
      200,
    )
  },
...
userRouter.get(
  GLOBAL_API_SCHEMAS.user.readUser.route.path,
  queryValidator(GLOBAL_API_SCHEMAS.user.readUser.query),
  async ctx => {
    const db = ctx.get('db')
    ...
    return apiSuccessResponse(
      GLOBAL_API_SCHEMAS.user.readUser.responseData,
      {
        id: result.id,
